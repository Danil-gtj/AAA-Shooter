...
void AGCAICharacterController::SetPawn(APawn* InPawn)
{
	...
	if (IsValid(InPawn)) 
	{
		checkf(InPawn->IsA<AGCAICharacter>(), TEXT("void AGCAICharacterController::SetPawn(APawn* InPawn) AICharacterController can possess on;y GCAICharacter"));
...
void AGCAICharacterController::TryMoveToNextTarget()
{
	...
	{
		FVector WayPoint = bIsPatrolling ? PatrollingComponent->SelectNextWaypoint() : PatrollingComponent->GetClosestWayPoint();
		if (IsValid(Blackboard))
		{
