...
void USaveSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    SaveDirectoryName = FString::Printf(TEXT("%sSaveGames/"), *FPaths::ProjectConfigDir());

    FSaveDirectoryVisitor DirectoryVisistor(SaveIds);
    FPlatformFileManager::Get().GetPlatformFile().IterateDirectory(*SaveDirectoryName, DirectoryVisistor);
    SaveIds.Sort();
...
void USaveSubsystem::WriteSaveToFile()
{
	...
	if (bUseCompressedSaves)
	{
		...
		FArchiveLoadCompressedProxy CompressedArchive(CompressedSaveBytes, NAME_Zlib);
...
void USaveSubsystem::OnActorSpawned(AActor* SpawnedActor)
{
	...
	if (IsValid(SpawnedActor) && SpawnedActor->Implements<USaveSubsystemInterface>() && SpawnedActor->HasActorBegunPlay())
	{
